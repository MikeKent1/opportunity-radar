-- Add server-side Grants and Tenders subcategory filters and counts.
...
create or replace function public.get_eligible_grant_subcategory_counts(
...
as $$
  with subcategories as (
    select *
    from (values
      ('high_value'), ('health'), ('social_services'), ('research'), ('education'),
      ('business'), ('environment'), ('eu_grants'), ('all')
  ...
  visible as (
    ...
    where o.status = 'active'
      ...
      and public.is_opportunity_in_feed(o, 'grants')
      and not exists (
  ...
  select
    ...
    count(visible.o) filter (
      where subcategories.subcategory_id = 'all'
        or public.is_opportunity_in_feed(visible.o, 'grants', subcategories.subcategory_id)
...
create or replace function public.get_eligible_tender_subcategory_counts(
  country_code text default null,
  profile_type text default null
)
returns table (subcategory_id text, opportunity_count bigint)
...
as $$
  with subcategories as (
    select *
    from (values
      ('high_value'), ('closing_soon'), ('norway'), ('sweden'), ('netherlands'),
      ('finland'), ('france'), ('all')
    ) as value(subcategory_id)
  ),
  visible as (
    ...
    where o.status = 'active'
      and public.is_opportunity_visible_for_user(o, country_code, profile_type)
      and public.is_opportunity_in_feed(o, 'tenders')
      and not exists (
        select 1 from public.hidden_opportunities h
        where h.opportunity_id = o.id and h.user_id = auth.uid()
      )
  ...
  select
    ...
    count(visible.o) filter (
      where subcategories.subcategory_id = 'all'
        or public.is_opportunity_in_feed(visible.o, 'tenders', subcategories.subcategory_id)
    ) as opportunity_count
  from subcategories
  left join visible on true
  group by subcategories.subcategory_id;
...
grant execute on function public.get_eligible_grant_subcategory_counts(text, text)
  to anon, authenticated;
grant execute on function public.get_eligible_tender_subcategory_counts(text, text)
  to anon, authenticated;
