-- Add server-side Free to Play subcategory filters and counts.
...
create or replace function public.get_eligible_freetoplay_subcategory_counts(
...
as $$
  ...
  visible as (
    ...
    where o.status = 'active'
      ...
      and public.is_opportunity_in_feed(o, 'freetoplay')
      and not exists (
        select 1
  ...
  select
    ...
    count(visible.o) filter (
      where subcategories.subcategory_id = 'all'
        or public.is_opportunity_in_feed(visible.o, 'freetoplay', subcategories.subcategory_id)
...
grant execute on function public.get_eligible_freetoplay_subcategory_counts(text, text)
  to anon, authenticated;
